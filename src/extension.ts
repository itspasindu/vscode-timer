...
let pauseButton: vscode.StatusBarItem;
...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('codingTimer.start', startTimer),
        vscode.commands.registerCommand('codingTimer.pause', pauseTimer),
        vscode.commands.registerCommand('codingTimer.stop', stopTimer),
        vscode.commands.registerCommand('codingTimer.dailyReport', generateDailyReport),
        vscode.commands.registerCommand('codingTimer.weeklyReport', generateWeeklyReport),
        vscode.commands.registerCommand('codingTimer.monthlyReport', generateMonthlyReport)
    );
...
function generateReport(period: 'day' | 'week' | 'month'): number {
    if (!fs.existsSync(logFilePath)) {
        return 0;
    }
    ...
    logData.forEach((entry: { date: string, duration: number }) => {
...
function isInSamePeriod(date1: Date, date2: Date, period: 'day' | 'week' | 'month'): boolean {
    if (period === 'day') {
